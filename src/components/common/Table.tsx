    ...
    <div className="overflow-x-auto p-4 transition-colors duration-300">
      ...
      >
        ...
        >
          <tr>
            {columns.map((col) => (
              <th key={col.key as string} className="text-left py-3 px-6 font-medium">
                {col.label}
              </th>
            ))}
            {renderActions && (
              <th
                className="py-3 px-6 font-semibold text-center tracking-wide 
        ...
        <tbody>
          {data.map((item) => (
            ...
            >
              {columns.map((col) => (
                <td key={col.key as string} className="py-3 px-6">
                  {String((item as any)[col.key])}
                </td>
              ))}
              {renderActions && (
                <td className="py-3 px-6 text-center">{renderActions(item)}</td>
              )}
            </tr>
          ))}
        </tbody>
